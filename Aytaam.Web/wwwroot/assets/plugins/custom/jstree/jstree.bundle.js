      ...
      if (obj && $.inArray(obj.id, this._model.changed) === -1) {
		...
		check : function (chk, obj, par, pos, more) {
			...
			if(chc === false || ($.vakata.is_function(chc) && chc.call(this, chk, obj, par, pos, more) === false) || (chc && chc[chk] === false)) {
				this._data.core.last_error = { 'error' : 'check', 'plugin' : 'core', 'id' : 'core_03', 'reason' : 'Orphan config for core.check_callback prevents function: ' + chk, 'data' : JSON.stringify({ 'chk' : chk, 'pos' : pos, 'obj' : obj && obj.id ? obj.id : false, 'par' : par && par.id ? par.id : false }) };
				return false;
			}
			return true;
		},
			...
			start : function (e, data, html) {
				...
				try {
					...
					if(e.currentTarget.style) {
						...
						e.currentTarget.style.MozOrphanSelect = "none";
					}
				} catch(ignore) { }
